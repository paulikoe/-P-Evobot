...
from beaker_coordinates import reagent_bakers_coord
...
distance_from_center = 15.5  # distance from the center of the petridish to pull decanol and salt
park_head_pos = (0, 0)
...
def compute_volumes(x_sol_ph, y_sol_ph, final_molarity, final_ph):
    ...
    print 'PH FIRST SOLUTION: %d' % x_sol_ph
    print 'PH SECOND SOLUTION: %d' % y_sol_ph
...
class EvolutionaryExperimentThirdFitness:
    ...
    def add_water(self, water_amount, petridish):
        ...
        self.water_pump.pumpPullVol(water_amount)
    ...
    def prepare_experiment(self, decanoate_solution, exp_petridish):
        ...
        x_sol_position = reagent_bakers_coord[x_sol_ph]
        y_sol_position = reagent_bakers_coord[y_sol_ph]
    ...
    def perform_experiment(self, petri, ph, molarity, gen, individual_num, replica_num):
        ...
        self.servo.servoMove(44)
        self.servo_syringe.dispenser_move(-26)
        ...
        while True:
            ...
            radius_start = 25
            analysis_frame = 10  # Number of frames between the current pos and the pos used to calculate the direction
            # Max angle that is allowed between the (droplet direction angle) and (droplet-salt angle)
            angle_threshold = 105.0 / 180.0 * math.pi

            distance_threshold_from_salt = 4.5
            ...
            if distance_from_start_point < radius_start and not left_safe_area:
                ...
                if counter % analysis_frame == 0.0:
                    ...
                    if len(path) - 1 - analysis_frame - frame_droplet_left_safe > -1:
                        ...
                        print 'THIS IS THE FITNESS COMPUTED EVERY %d STEP: %f' % (
                        analysis_frame, (fitness_value / frame_counter))

                    salt_distance = _calculate_pitagora(current_pos, salt_destination_pixel)
                    ...
                    if salt_distance < distance_threshold_from_salt:
        ...
        if frame_counter == 0.0:
            ...
            fitness_value = fitness_value / (frame_counter + 1)
        ...
        import csv
        with open('/Users/capo/Documents/experiments_evolution2/' + file_name_addition + "-{}.csv".format(
