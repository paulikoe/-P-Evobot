...
class EvolutionaryExperiment:
    ...
    def perform_experiment(self, petri, ph, molarity, gen, individual_num, replica_num):
            ...
            if distance_from_start_point < radius_start and not left_safe_area:
                ...
                if counter % analysis_frame == 0.0:
                    ...
                    if len(path) - 1 - analysis_frame - frame_droplet_left_safe > -1:
                        ...
                        # print 'THIS IS THE FITNESS COMPUTED: %f' % (fitness_value / frame_counter)
        ...
        if recording:
            import os
            date = str(datetime.datetime.now())
            file_name = os.path.join(FILE_PATH, "%s_fitness%f.jpg" % (vT.removeColon(date + file_name_addition), fitness_value))
        ...
        if edge_check > limit_edge:
            # the end point is without the distance
            fitness_value = 0.0
            print 'at border'
        else:
            print 'not at border'
